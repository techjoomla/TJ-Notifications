	...
	function addPreferance(pId,client,provider,key)
	{
		if(pId)
		{
			jQuery.ajaxSetup({
				global: false,
				type:'post',
				url:'<?php echo JURI::root();?>index.php?option=com_tjnotifications&task=preferences.save',
				dataType:'json',
				beforeSend: function () {
	...
	function removePreferance(pId,client,provider,key)
	{
		 if(pId)
		 {
			jQuery.ajaxSetup({
				global: false,
				type:'post',
				url:'<?php echo JURI::root();?>index.php?option=com_tjnotifications&task=preferences.delete',
